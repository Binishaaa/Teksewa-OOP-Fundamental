//using if-else
Console.WriteLine("Enter the number of electricity units consumed:");
string userInput = Console.ReadLine();
int unit;
decimal totalAmount = 0;
if (int.TryParse(userInput,out unit))
{
    if (unit >=1 && unit <= 20)
    {
        totalAmount = 100;
    ...
    Console.WriteLine($"The total amount is Rs {totalAmount}");
}
...
if (int.TryParse(Input, out units))
{
    switch (units)
    {
    ...
    Console.WriteLine($"The total amount is Rs {totalAmount1}");
}
